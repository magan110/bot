...
public class GeminiSqlGenerator : ISqlGenerator
{
    ...
    private readonly ILogger<GeminiSqlGenerator> _logger;

    public GeminiSqlGenerator(
        HttpClient httpClient,
        IConfigurationService configService,
        ILogger<GeminiSqlGenerator> logger)
    {
        _httpClient = httpClient;
        _apiKey = configService.GetSetting<string>("Bot:Gemini:ApiKey") ?? "";
        _model = configService.GetSetting<string>("Bot:Gemini:Model") ?? "gemini-pro";
        _logger = logger;
    }
        ...
        ConversationContext context)
    {
        try
        {
            _logger.LogInformation("Generating SQL with Gemini for query: {Query}", naturalLanguageQuery);
        ...
        catch (Exception ex)
        {
            ...
            throw new LlmProviderException($"Gemini API error: {ex.Message}");
        }
    }
