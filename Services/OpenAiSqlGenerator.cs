...
public class OpenAiSqlGenerator : ISqlGenerator
{
    ...
    public OpenAiSqlGenerator(
        ...
        ILogger<OpenAiSqlGenerator> logger)
    {
        _httpClient = httpClient;
        _apiKey = configService.GetSetting<string>("Bot:OpenAI:ApiKey") ?? "";
        _model = configService.GetSetting<string>("Bot:OpenAI:Model") ?? "gpt-4";
        _baseUrl = configService.GetSetting<string>("Bot:OpenAI:BaseUrl") ?? "https://api.openai.com/v1";
        _logger = logger;
        ...
        ConversationContext context)
    {
        try
        {
            _logger.LogInformation("Generating SQL for query: {Query}", naturalLanguageQuery);
            
            var language = DetectLanguage(naturalLanguageQuery);
            var systemPrompt = BuildSystemPrompt(schema, language);
        ...
        catch (Exception ex)
        {
            ...
            throw new LlmProviderException($"OpenAI API error: {ex.Message}");
        }
    }
    ...
    private string BuildSystemPrompt(DatabaseSchema schema, string language)
    {
